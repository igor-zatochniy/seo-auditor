...
- Маскування чутливих query-параметрів URL у логах, помилках і `safe_url`; унікальність результатів тримається на HMAC `target_fingerprint`.
...
```bash
docker compose exec postgres psql -U seo_user -d seo_db -c "SELECT run_id, safe_url, status_code, scan_status, robots_outcome, error_code FROM audit_results ORDER BY created_at DESC LIMIT 10;"
```
...
```bash
docker compose exec postgres psql -U seo_user -d seo_db -c "SELECT run_id, safe_url, error_code, error_message, created_at FROM audit_results WHERE scan_status = 'failed' AND run_id = (SELECT id FROM audit_runs ORDER BY started_at DESC LIMIT 1) ORDER BY created_at DESC;"
```
