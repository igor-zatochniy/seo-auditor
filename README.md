...
- Correlation `run_id` у кожному log record і збереженому результаті аудиту.
...
│   ├── volume: pgdata
│   ├── init SQL baked from initdb/01_init.sql
...
└── parser
    ...
    └── upserts metrics into seo_results
```
...
| `RUN_ID` | generated | Необов'язковий correlation identifier; якщо відсутній, генерується криптографічно. |
...
Після `SIGTERM` parser завершує in-flight задачі в межах `SHUTDOWN_TIMEOUT`, фіксує відкладені результати в логах і повертає exit code `2`.

Для наявного локального PostgreSQL volume ідемпотентно застосуйте актуальні outcome-поля без видалення даних:

```powershell
Get-Content -Raw .\initdb\01_init.sql | docker compose exec -T postgres sh -c 'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB"'
```
...
```bash
docker compose exec postgres psql -U seo_user -d seo_db -c "SELECT run_id, url, status_code, scan_status, robots_outcome, error_code FROM seo_results ORDER BY created_at DESC LIMIT 10;"
```

Перегляд невдалих задач останнього запуску:

```bash
docker compose exec postgres psql -U seo_user -d seo_db -c "SELECT run_id, url, error_code, error_message, created_at FROM seo_results WHERE scan_status = 'failed' ORDER BY created_at DESC;"
```
