...
func Load() (Config, error) {
	...
	if targetLeaseDuration <= auditRunHeartbeatInterval {
		return Config{}, fmt.Errorf("TARGET_LEASE_DURATION must be greater than AUDIT_RUN_HEARTBEAT_INTERVAL")
	}
	if targetLeaseDuration <= robotsTotalTimeout+httpTotalTimeout {
		return Config{}, fmt.Errorf("TARGET_LEASE_DURATION must exceed ROBOTS_TOTAL_TIMEOUT plus HTTP_TOTAL_TIMEOUT")
	}
	...
	maxHTMLBodyBytes, err := int64FromEnv("MAX_HTML_BODY_BYTES", DefaultMaxHTMLBodyBytes, 1024, MaxHTMLBodyBytes)
	if err != nil {
		return Config{}, err
	}
